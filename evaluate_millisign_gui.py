"""
Evaluate trained DreamerV3 MilliSign model with PyBullet GUI
"""
import argparse
import pathlib
import sys
...
class MilliSignEnvGUI:
    """MilliSign environment with GUI visualization."""
    ...
    def _create_markers(self):
        ...
        # MilliSign tag (small box at chest)
        ...
        # Radar beam visualization (cone)
        self._beam_visual = p.createVisualShape(
            p.GEOM_MESH,
            fileName="",
            meshScale=[1, 1, 1],
            rgbaColor=[0.0, 0.5, 1.0, 0.2],
            physicsClientId=self._env.CLIENT
        )
    ...
    def observation_space(self):
        import gym
        # HoverAviary KIN obs: 72 dims + radar(5) + person_vel(3) + target(3) = 83 dims
    ...
    def reset(self):
        ...
        # HoverAviary returns (1, 72) obs - flatten to (72,)
        obs_flat = np.array(obs, dtype=np.float32).flatten()  # Shape: (72,)
        drone_pos = obs_flat[:3]
        tag_pos = self._get_tag_position()
        radar_obs = self._simulate_radar_detection(drone_pos, tag_pos)

        # Total: 72 + 5 + 3 + 3 = 83 dims
        extended_obs = np.concatenate([
            obs_flat,  # 72 dims from HoverAviary
            radar_obs,  # 5 dims
            np.zeros(3, dtype=np.float32),  # person_vel: 3 dims
            np.array([self._target_distance, self._target_height, 0.0], dtype=np.float32),  # target: 3 dims
        ])
    ...
    def step(self, action):
        ...
        extended_obs = np.concatenate([
            obs_flat,  # 72 dims
            radar_obs,  # 5 dims
            self._person_vel.astype(np.float32),  # 3 dims
            np.array([self._target_distance, self._target_height,
                     radar_obs[0] - self._target_distance if radar_obs[4] > 0.5 else 0.0], dtype=np.float32),  # 3 dims
        ])
...
def main():
    ...
    parser.add_argument("--slow", type=float, default=1.0, help="Slowdown factor (1.0 = realtime)")
    ...
    print("\n" + "="*50)
    print("MilliSign Person Following - GUI Evaluation")
    print("="*50)
    print("Green cylinder = Person")
    print("Red box = MilliSign tag (mmWave reflector)")
    print("Drone should follow at ~3m distance")
    print("="*50 + "\n")
    ...
    for ep in range(args.episodes):
        ...
        while not done and step < 300:
            ...
            # Add dummy image (not used by state-based model but needed for preprocess)
